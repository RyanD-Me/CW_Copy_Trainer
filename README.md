...
- 練習: 英数字・略符号・実在局コールサイン・各コンテストのナンバーを繰り返し再生して聞き取り
- スコアアタック: 練習 / コールサイン / コンテストナンバー / コンテストの4モード、モード別ハイスコア
- コンテストモードは実際のコンテスト公開ログにある「コールサインとナンバーの組」から出題

開発者向けの仕様・ルールは `CLAUDE.md` を参照してください。

```
python3 tools/build_data.py          # data/（ログ含む）から出題データを再生成して index.html に埋め込む
